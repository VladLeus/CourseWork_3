...
const CartElemCard: React.FC<CartElemCardProps> = ({ carPart}) => {
    ...
    return (
        <div className="flex flex-col items-start justify-center w-[250px] rounded-2xl bg-white text-black font-normal m-2">
            ...
            <p className="ml-1.5"><span className="font-custom">Quantity:</span> <span className="font-bold cursor-pointer" onClick={incrementQuantity}>+</span> {quantity} <span className="font-bold cursor-pointer" onClick={decrementQuantity}>-</span></p>
            <p onClick={handleRemoveFromCart} className="bg-black text-white font-thin py-1 px-2 rounded-lg ml-1.5 my-0.5 cursor-pointer hover:bg-rojo">Remove</p>
        </div>
    )
}
